...
export default function StockReceive(){
  ...
  console.log(item);
    ...
    setTableData(tableData=>([...tableData, formData]));
     ...
     const emptytabledata = ()=>{
      ...
      axios.get('http://localhost/bills_online/commons/stock_management_endpoint/index.php/receive/createstrg',{
   ...
   export function AddTableData(props){
    ...
    return  (
     ...
     <form onSubmit ={receiveApproval}>
         <h2>Stock Receive Details</h2>
         <table>
      <thead>
 ...
 export function AddFormData(props){
  ...
  const [formData, setFormData] = useState({id:"", eventid:"" , item:"", narration:"",
  quantity: "", date:"", batch:1234, location:""});
  const [store, setStore] = useState([]);
    const [drug, setDrug] = useState([]);
    
    
    useEffect(()=>{axios.get('http://localhost/bills_online/commons/stock_management_endpoint/index.php/receive/list?location=1021&trans_id=120220000039131')
  .then(Response=>{
  ...
  var data = Response.data;
   ...
   const handlereceive =  ()=>{
    ...
    axios.get('http://localhost/bills_online/commons/stock_management_endpoint/index.php/receive/createevt',{
  ...
  return (
    <div>
      <h3>New Stock Receive</h3>
      <button type ="button" name ="eventid" onClick={handlereceive} >+</button>
   ...
   <input type ='text' name='narration' placeholder ='Enter narration' onChange ={handlechange}   value = {formData.narration} required/>
  ...
  <input type ='number' name='quantity' placeholder ='Enter quantity' onChange ={handlechange}   value= {props.quantitycount} required/>
    ...
    <input type ='text' name='unit_cost' placeholder ='Enter unit cost' onChange ={handlechange}   value= {formData.unit_cost} required/>
  ...
  var data = Response.data;
   ...
   {drug.map((dr)=>(
  <option key ={dr.id} value ={dr.naming}>{dr.naming}</option>
   )
     )
    }
   </select>
